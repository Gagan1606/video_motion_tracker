...
Reason to deviate from the given pipeline:
    ...
    And also morphology ops on edges is less accurate as compared blobs, so it makes sense to do morphology with bg mask and find the edges right before contour selection.
```
